		...
		if (Input.GetMouseButton (0) ) {
			...
			if (clawLeft.transform.rotation.z <= 0) {
				if (!clawLeft.GetComponent<ClawHit>().hit)
					clawLeft.transform.Rotate (clawLeft.transform.rotation.x, clawLeft.transform.rotation.y, clawLeft.transform.rotation.z + 5f);
				if (!clawRight.GetComponent<ClawHit>().hit)
					clawRight.transform.Rotate (clawRight.transform.rotation.x, clawRight.transform.rotation.y, clawRight.transform.rotation.z - 5f);
			}
